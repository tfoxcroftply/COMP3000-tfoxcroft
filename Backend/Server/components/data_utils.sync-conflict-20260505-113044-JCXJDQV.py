...
class DataUtils:
    ...
    async def retrieve_data(self, requested_time: int, node_hwid: int | None = None, type: DataTypeEnum = DataTypeEnum.ALL, starts_from: int | None = None) -> DataObject | None: # time is in mins. maybe add some sort of caching later. DOES NOT RETURN HWID
        ...
        current_time = starts_from if starts_from is not None else int(time())
        print(requested_time)

        found: ReturnData

        if node_hwid is not None:
            found = await self.container.database.read_all(f"SELECT * from readings WHERE node_hwid = ? AND timestamp <= ? AND timestamp >= ?",(node_hwid, current_time, current_time - requested_time * 60 * 60,))
        else:
            found = await self.container.database.read_all(f"SELECT * from readings WHERE timestamp <= ? AND timestamp >= ?",(current_time, current_time - requested_time * 60 * 60,))
